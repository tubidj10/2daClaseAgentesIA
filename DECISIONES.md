...
## Iteración 10 — Manejo de errores de parseo y fórmula de costo en el README

Feedback recibido (revisión de código, no puntuada + dimensión de análisis económico):

- **Real, corregido:** `json.loads(texto)` en `ejecutar()` no tenía manejo específico de `json.JSONDecodeError` — si el modelo devolvía un JSON truncado (como pasó de verdad en la Iteración 7), el traceback crudo de `json.loads` subía sin contexto. Se extrajo la lógica a `_parsear_ticket(texto)`, que envuelve el parseo y levanta un `RuntimeError` descriptivo (qué pasó, dónde mirar, el texto crudo recibido) en vez del traceback pelado. Se agregó `test_parsear_ticket_json_truncado_da_error_descriptivo`, que reproduce el caso real. No se agregó un reintento automático (una de las dos alternativas que sugería el feedback): la causa raíz ya tiene su fix (`thinking_budget`, Iteración 7) — reintentar sobre el mismo prompt sin cambiar nada no resolvería un JSON truncado, solo lo escondería.
- **Real, corregido:** el README principal no tenía la fórmula de costo desagregada ni el rango mínimo-máximo — solo linkeaba a `COSTOS.md`. Se agregó una sección "Costo por corrida" con la fórmula (`tokens_input × $1.50/1M + tokens_output × $9.00/1M`) y el rango real medido (mínimo $0.0084, máximo $0.0110, sobre las 3 corridas automatizadas — no una proyección).
- **Verificado y descartado como falso positivo:** el feedback también decía que `corridas/corrida-manual-4-entrega3-herramienta.md` "es citado en DECISIONES.md pero no existe en el repositorio". Se verificó contra `origin/main` (`git ls-tree origin/main -r`): el archivo existe, está trackeado desde el commit `c127435`. No se aplicó ningún cambio por este punto — no correspondía.
